...
namespace everest {
    namespace connection {
        namespace serial_connection_helper {
            ...
            ::size_t read_from_device( int serial_port_fd,
                                       unsigned char* buffer,
                                       ::size_t count ,
                                       termios* tty_config ,
                                       unsigned int initial_timeout_deciseconds,
                                       unsigned int timeout_deciseconds
                );
        }
    }
}
...
bool operator == ( const termios& lhs, const termios& rhs ) {
    return
        ( lhs.c_cflag  == rhs.c_cflag  ) and
        ( lhs.c_iflag  == rhs.c_iflag  ) and
        ( lhs.c_ispeed == rhs.c_ispeed ) and
        ( lhs.c_lflag  == rhs.c_lflag  ) and
        ( lhs.c_line   == rhs.c_line   ) and
        ( lhs.c_oflag  == rhs.c_oflag  ) and
        ( lhs.c_ospeed == rhs.c_ospeed );
}

namespace ecs = everest::connection::serial_connection_helper;

everest::connection::SerialDeviceConfiguration::SerialDeviceConfiguration( std::string device ) :
    m_device( device ) {
...
::size_t everest::connection::SerialDevice::read( unsigned char* buffer, ::size_t count ) {
    ...
    return ::ecs::read_from_device( m_fd,
                                                                              buffer,
                                                                              count,
                                                                              &get_serial_device_config().m_tty_config,
...
int ecs::open_serial_device( const std::string& device ) {
    ...
    throw everest::connection::exceptions::tty::tty_error( "Error open serial device: " + device + " Reason: " + strerror( errno ) , errno );
...
void ecs::get_default_configuration( int serial_port_fd, termios* tty ) {
    ...
    if( tcgetattr(serial_port_fd, tty) != 0 ) {
        int myerror = errno;
        throw everest::connection::exceptions::tty::tty_error( "Error " + std::to_string( myerror ) + " from tcgetattr: " + strerror(myerror) , myerror );
    }
...
void ecs::set_baudrate( termios* tty, everest::connection::SerialDeviceConfiguration::BaudRate baudrate ) {
    cfsetspeed( tty , static_cast<std::underlying_type_t<everest::connection::SerialDeviceConfiguration::BaudRate> >( baudrate ) );
}
...
void ecs::configure_device( int serial_port_fd, termios* tty ) {
    ...
    if ( tcsetattr(serial_port_fd, TCSANOW, tty) != 0) {
        int myerror = errno;
        throw everest::connection::exceptions::tty::tty_error("Error " + std::to_string( myerror ) + " from tcsetattr: " + strerror( myerror ) , myerror );
    }
}
...
::size_t ecs::write_to_device( int serial_port_fd, const unsigned char* const buffer, ::size_t count ) {
    ...
    while ( bytes_written_sum < count ) {
        ...
        if ( bytes_written == -1 ) {
            // handle error
            int myerror = errno;
            throw everest::connection::exceptions::tty::tty_error( "Error: " + std::to_string( myerror ) + " from ::write: " + strerror( myerror ) , myerror );
        }
...
::ssize_t readByteFromDevice( int fd, unsigned char* charToRead ) {
    ...
    int myerror = errno;
    throw everest::connection::exceptions::tty::tty_error("Error: " + std::to_string( myerror ) + " from ::read: " + strerror( myerror ) , myerror );

}


::size_t ecs::read_from_device( int serial_port_fd,
                                                                          unsigned char* buffer,
                                                                          ::size_t count ,
                                                                          termios* tty_config,
                                                                          unsigned int initial_timeout_deciseconds,
...
void everest::connection::SerialDeviceConfiguration::SerialDeviceConfiguration::get_current_config() {
    ...
    if(tcgetattr(serial_port, &m_tty_config) != 0) {
        throw everest::connection::exceptions::tty::tty_error( ""s + __PRETTY_FUNCTION__ + "Error : " + strerror( errno ) , errno );
    }
    close( serial_port );
}


everest::connection::SerialDeviceConfiguration& everest::connection::SerialDeviceConfiguration::set_baud_rate( BaudRate baudrate ){
    cfsetspeed( &m_tty_config, static_cast<std::underlying_type_t<everest::connection::SerialDeviceConfiguration::BaudRate> >( baudrate ) );
    return *this;
}


everest::connection::SerialDeviceConfiguration& everest::connection::SerialDeviceConfiguration::set_data_bits( DataBits data_bits ) {
    m_tty_config.c_cflag &= ~CSIZE; // clear bits
    m_tty_config.c_cflag |= static_cast<std::underlying_type_t<everest::connection::SerialDeviceConfiguration::DataBits> > ( data_bits ) ;
    return *this;
}


everest::connection::SerialDeviceConfiguration& everest::connection::SerialDeviceConfiguration::set_stop_bits( StopBits stop_bits ) {
...
everest::connection::SerialDeviceConfiguration& everest::connection::SerialDeviceConfiguration::set_parity( Parity parity ) {
...
everest::connection::SerialDeviceConfiguration& everest::connection::SerialDeviceConfiguration::set_sensible_defaults() {
